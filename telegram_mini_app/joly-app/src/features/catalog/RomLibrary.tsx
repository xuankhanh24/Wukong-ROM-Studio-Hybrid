...
export function RomLibrary({ live, fallbackDevices = [], onToast, onUseSource }: { live: boolean; fallbackDevices?: RomLibraryFallbackDevice[]; onToast: (message: string) => void; onUseSource: (uri: string) => void }) {
  ...
  useEffect(() => {
    if (live || !fallbackDevices.length) return;
    const next = fallbackDevices.map(normalizeDevice).filter((device): device is RomDevice => Boolean(device));
    if (next.length) setDevices(next);
  }, [fallbackDevices, live]);
  ...
  const emptyMessage = status === "ready" ? "Không có release phù hợp." : "Chưa có release được chọn.";
  const selectRelease = (release: RomRelease) => setSelectedId(release.id);
  const useRelease = (release: RomRelease) => { if (release.sourceUrl) onUseSource(release.sourceUrl); };

  return <section className="panel rom-library" aria-labelledby="rom-library-title"><div className="panel-head"><div><span className="eyebrow">ROM LIBRARY / SOURCE RESOLVER</span><h2 id="rom-library-title">Tìm ROM theo thiết bị</h2><p>Tra cứu release theo region/version rồi đưa source đã chọn về Studio.</p></div><span className="mono">{devices.length} devices</span></div><div className="rom-library-controls"><label>Device search<div className="search-field"><Search size={15} /><input value={deviceQuery} onChange={(event) => setDeviceQuery(event.target.value)} placeholder="OnePlus / OPPO / model…" /></div></label><label>Device<select value={selectedDevice} onChange={(event) => { setSelectedDevice(event.target.value); setRegion(""); setStatus("idle"); }}><option value="">Chọn thiết bị</option>{visibleDevices.map((device) => <option key={device.id} value={device.id}>{device.label} · {device.id}</option>)}</select></label><label>Region<select value={region} onChange={(event) => setRegion(event.target.value)} disabled={!currentDevice}><option value="">Tất cả region</option>{(currentDevice?.regions || []).map((entry) => <option key={entry.code} value={entry.code}>{entry.code}{entry.models.length ? ` · ${entry.models.join(", ")}` : ""}</option>)}</select></label><Button onClick={() => void search()} disabled={!selectedDevice || status === "loading"}><Search size={15} /> {status === "loading" ? "Đang tìm" : "Tìm release"}</Button></div><div className="rom-library-status" role="status" aria-live="polite">{status === "idle" ? "Chọn device để bắt đầu." : status === "loading" ? "Đang tải release…" : status === "error" ? "Không tải được release; thử lại." : `${rows.length} release phù hợp`}</div><AnimatedTable className="rom-desktop-table" data={rows} columns={columns} loading={status === "loading"} loadingRows={2} emptyMessage={emptyMessage} onRowClick={(row) => selectRelease(row)} /><div className="rom-mobile-list" aria-label="Danh sách release trên mobile">{status === "loading" ? <div className="rom-mobile-empty">Đang tải release…</div> : rows.length === 0 ? <div className="rom-mobile-empty">{emptyMessage}</div> : rows.map((row) => <article key={row.id} className={`rom-mobile-row ${selectedId === row.id ? "is-selected" : ""}`} onClick={() => selectRelease(row)}><div className="rom-mobile-row-head"><strong>{row.version}</strong><span className="status-pill status-neutral"><i />{row.region}</span></div><p>{row.device} · {row.model}</p><code className="mono">{row.securityPatch || "—"} · {formatBytes(row.sizeBytes)}</code><div className="rom-mobile-row-actions"><Button size="sm" variant="outline" onClick={(event) => { event.stopPropagation(); void navigator.clipboard?.writeText(row.sourceUrl); onToast("Đã sao chép source URL."); }}><Copy size={14} /> Sao chép</Button><Button size="sm" disabled={!row.sourceUrl} onClick={(event) => { event.stopPropagation(); useRelease(row); }}>Dùng trong Studio <ArrowUpRight size={14} /></Button></div></article>)}</div><div className="rom-library-detail">{selectedId && <><span className="eyebrow">SELECTED RELEASE</span><strong>{releases.find((item) => item.id === selectedId)?.version}</strong><div className="rom-library-actions"><Button size="sm" variant="outline" onClick={() => void resolve()} disabled={resolving}>{resolving ? <RefreshCw size={14} className="spin" /> : <ArrowUpRight size={14} />} {resolving ? "Resolving…" : "Resolve source"}</Button><Button size="sm" onClick={() => { const release = releases.find((item) => item.id === selectedId); if (release) useRelease(release); }}>Dùng trong Studio</Button></div>{resolvedUrl && <code className="mono rom-resolved-url">{resolvedUrl}</code>}</>}</div></section>;
}
